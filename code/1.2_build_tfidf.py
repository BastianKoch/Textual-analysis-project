"""
Build Bigram TF-IDF Matrices
=============================

Build three bigram TF-IDF sparse matrices over all earnings-call
transcripts: combined (presentation + answers), pres, and answers.

    tfidf(b, t) = f(b, t) * log(T / df(b))

where f(b, t) is raw bigram frequency in transcript t, T is the total
number of transcripts, and df(b) is the document frequency of bigram b.

Input
-----
    data/interim/bigrams/{year}/*_{segment}.txt

Output
------
    data/processed/bigram_tfidf_{label}.npz        (scipy CSR float32)
    data/processed/bigram_vocab_{label}.csv        (index, bigram)
    data/processed/bigram_transcript_index.csv     (row, transcript_id)

Usage
-----
    python code/1.2_build_tfidf.py
