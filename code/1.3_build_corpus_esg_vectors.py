"""
Build ESG Corpus TF-IDF Vectors
================================

Project each ESG reference corpus (E, S, G) into the bigram TF-IDF
space built from the earnings-call transcripts.

    tfidf(b, c) = freq(b, c) * log(T / df_transcript(b))

where freq(b, c) is the raw bigram count in corpus c, T is the number
of transcripts, and df_transcript(b) is the transcript document frequency.

Input
-----
    data/processed/Text corpus/{E,S,G}/*.txt
    data/processed/bigram_tfidf_{label}.npz
    data/processed/bigram_vocab_{label}.csv

Output
------
    data/processed/esg_corpus_vectors/corpus_tfidf_{label}_{E|S|G}.npz
    data/processed/esg_corpus_vectors/corpus_stats_{label}.csv

Usage
-----
    python code/1.3_build_corpus_esg_vectors.py
